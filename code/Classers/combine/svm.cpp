...
voter svm_predict_mod(const svm_model *model, const svm_node *x)
{
	voter vote_info;
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
...
/*		if(model->param.svm_type == ONE_CLASS)
	...
	else
	{
		int i;
		int nr_class = model->nr_class;
		vote_info.vote = new int[nr_class];					//modified
		...
		for(i=0;i<nr_class;i++)
			if(vote[i] > vote[vote_max_idx])
				vote_max_idx = i;
		for(i=0;i<nr_class;i++)				//modified
			vote_info.vote[i] = vote[i];
		free(vote);
		free(dec_values);
		return vote_info; //modified
