...
KeypointDetector::KeypointDetector(char * fn) {
	...
	if(!pcaf)
	{
		printf("pca vectors file read-error! please check the file-path.");
		return;
	}
...
void tttt(vector<vector<Image *> > GOctaves)
{
	vector<vector<Image *> > veTemp = GOctaves; 
	veTemp.swap( GOctaves ); 
