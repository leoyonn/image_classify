				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	octvs = log( MIN( init_img->width, init_img->height ) ) / log(2) - 2; //log: natural logarithm
			...
			for(r = SIFT_IMG_BORDER; r < dog_pyr[o][0]->height-SIFT_IMG_BORDER; r++)
				for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c++)
					...
					if( ABS( pixval32f( dog_pyr[o][i], r, c ) ) > prelim_contr_thr )
						...
						{
							feat = interp_extremum(dog_pyr, o, i, r, c, intvls, contr_thr); //对比度判断
							if( feat )
							{
								ddata = feat_detection_data( feat );
								if( ! is_too_edge_like( dog_pyr[ddata->octv][ddata->intvl], 
									ddata->r, ddata->c, curv_thr ) )//边缘点判断
								{
...
@param xi output as interpolated subpixel increment to interval->sigma
@param xr output as interpolated subpixel increment to row->y
@param xc output as interpolated subpixel increment to col->x
*/
					 ...
					 double scl, int d, int n ) //d=4, n=8
{
	...
	for( i = 0; i < d; i++ )
	{
		...
		for( j = 0; j < d; j++ )
			hist[i][j] = (double *)calloc( n, sizeof( double ) ); //16个种子点
	}
